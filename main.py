...
def get_keywords(jd_require, jd_optional):
    ...
    if jd_require:
        ...
        for sentence in required_sentences:
            required_keywords.extend([word for word, tag in okt.pos(sentence+sentence.replace(" ","")) if tag in ["Noun", "Alpha"]])
    if jd_optional:
        # 지원자격의 문장을 줄바꿈으로 나눕니다.
        optional_sentences = jd_optional.split("\n")
        # 각 문장을 형태소 분석기로 분리하고, 명사와 영어만 키워드로 추출합니다.
        for sentence in optional_sentences:
            optional_keywords.extend([word for word, tag in okt.pos(sentence+sentence.replace(" ","")) if tag in ["Noun", "Alpha"]])
